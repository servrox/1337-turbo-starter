...
export function UnstakeTimelockedObjectsView({
  ...
  const validatorInfo = systemState?.activeValidators?.find(
    ({ iotaAddress: validatorAddress }) => validatorAddress === groupedTimelockedObjects.validatorAddress
  );
    ...
    <DialogLayout>
      ...
      <DialogLayoutBody>
        <div className="gap-md flex flex-col">
          <Validator address={groupedTimelockedObjects.validatorAddress} isSelected showActiveStatus />
          ...
          <StakeRewardsPanel stakingRewards={totalRewards} totalStaked={totalStakedAmount} isTimelocked />
...
function TimelockedStakeCollapsible({ stake, title, handleCopySuccess }: TimelockedStakeCollapsibleProps) {
  ...
  return (
    <Collapsible defaultOpen key={stake.timelockedStakedIotaId} title={title}>
      <Panel>
        <div className="gap-y-sm p-md--rs py-sm flex flex-col">
          <KeyValueInfo
            keyText="Stake ID"
            ...
            fullwidth
          />
          <KeyValueInfo keyText="Expiration time" value={currentEpochEndTimeAgo} fullwidth />
          {stake.label && (
            <KeyValueInfo
              keyText="Label"
