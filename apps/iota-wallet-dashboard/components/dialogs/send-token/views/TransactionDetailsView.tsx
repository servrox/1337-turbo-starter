...
import { useGetTransaction } from "@repo/iota-core";
...
export function TransactionDetailsView({ digest, onClose }: TransactionDetailsViewProps) {
  ...
  if (isError) {
    return (
      <InfoBox
        ...
        supportingText={error?.message ?? "An error occurred when getting the transaction info"}
        icon={<Warning />}
  ...
  return transaction ? <TransactionDetailsLayout transaction={transaction} onClose={onClose} /> : null;
}
