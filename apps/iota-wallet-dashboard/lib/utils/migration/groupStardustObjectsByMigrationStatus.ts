...
import { CommonOutputObjectWithUc, MILLISECONDS_PER_SECOND } from "@repo/iota-core";
...
export function groupStardustObjectsByMigrationStatus(
  ...
  for (const outputObject of stardustOutputObjects) {
    ...
    if (outputObjectFields.timelock_uc && outputObjectFields.timelock_uc.fields.unix_time > epochUnix) {
...
export function getStardustObjectsTotals({
  ...
  if (resolvedObjects) {
    filteredObjects = filterMigrationObjects(resolvedObjects, StardustOutputDetailsFilter.NativeTokens);
  }
  ...
  for (const output of outputObjects) {
    ...
    totalIotaAmount += extractOwnedStorageDepositReturnAmount(outputObjectFields, address) || 0n;
...
export function extractOwnedStorageDepositReturnAmount(
  ...
  if (storage_deposit_return_uc?.fields && storage_deposit_return_uc?.fields.return_address === address) {
...
export function getUniqueNativeTokensByCoinType(objects: ResolvedObjectTypes[]): number {
  return new Set(
    objects.filter((obj) => obj.commonObjectType === CommonMigrationObjectType.NativeToken).map((obj) => obj.coinType)
  ).size;
}
